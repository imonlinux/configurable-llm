"""Anthropic Messages API provider for Configurable LLM.

This module is the Anthropic backend behind the :class:`LLMProvider` interface.
It holds the request/response/streaming logic that previously lived inline in
``entity.py`` and ``coordinator.py``; the behavior is unchanged — only
relocated behind the provider seam.
"""
...
class AnthropicProvider(LLMProvider):
    ...
    def categorize_error(self, err: Exception) -> ProviderError:
        ...
        if isinstance(err, anthropic.APIStatusError):
            # The SDK maps 401 to AuthenticationError, but some Anthropic-compatible
            # servers return a generic APIStatusError carrying an auth error body.
            body = getattr(err, "body", None)
            if (
                isinstance(body, dict)
                and (error := body.get("error"))
    ...
    async def async_list_models(self, client: anthropic.AsyncAnthropic) -> list[ModelInfo]:
        """Fetch the model list, raising HA-level exceptions on failure."""
        try:
            result = await client.models.list(timeout=10.0)
    ...
    async def fetch_model(
        ...
        """Resolve a model id not in the cached list via ``models.retrieve``."""
        try:
            model_info = await coordinator.client.models.retrieve(
    ...
    async def build_request(
        ...
        """Build the Anthropic Messages request kwargs for one turn."""
        options: dict[str, Any] = ctx.options
        ...
        messages, container_id = _convert_content(ctx.chat_log.content[1:])

        model = ctx.model.id

        model_args: MessageCreateParamsStreaming = MessageCreateParamsStreaming(
            model=model,
            messages=messages,
        ...
        if (
            ctx.model.capabilities
            and ctx.model.capabilities.thinking.types.adaptive.supported
        ):
            ...
            if (
                ctx.model.capabilities
                and ctx.model.capabilities.thinking.types.enabled.supported
            ...
            if (
                ctx.model.capabilities
                and ctx.model.capabilities.effort.supported
            ):
        ...
        if options[CONF_CODE_EXECUTION]:
            ...
            if (
                not ctx.model.capabilities
                or not ctx.model.capabilities.code_execution.supported
        ...
        if ctx.structure and structure_name:
            if (
                ctx.model.capabilities
                and ctx.model.capabilities.structured_outputs.supported
            ):
            ...
            elif model_args["thinking"]["type"] == "disabled":  # type: ignore[index]
    ...
    async def create_stream(
        self, client: anthropic.AsyncAnthropic, request_kwargs: dict[str, Any]
    ) -> AsyncStream[MessageStreamEvent]:
        """Issue the streaming Messages request."""
    ...
    def make_transformer(
        ...
        *,
        ...
        """Wrap the raw Anthropic stream with the delta converter."""
        return ConfigurableLLMDeltaStream(chat_log, stream, output_tool=output_tool)

    def convert_back(
        self, chat_content: Iterable[conversation.Content]
    ) -> tuple[list[MessageParam], str | None]:
        """Convert HA chat content into Anthropic messages (+ container state)."""
        return _convert_content(chat_content)
    ...
    def merge_iteration_state(
        ...
        """Fold one iteration's messages back, carrying the container id."""
        request_kwargs["messages"].extend(new_messages)
        request_kwargs["container"] = state
    ...
    async def prepare_files(
        ...
        """Build Anthropic-native attachment blocks."""
        return await _async_prepare_files_for_prompt(hass, model, files)

    def deprecated_models(self) -> dict[str, str]:
        """Mapping of deprecated Anthropic model id -> retirement date."""
        return DEPRECATED_MODELS

    def is_model_deprecated(self, model_id: str) -> bool:
        """Whether an Anthropic model id is deprecated."""
        return model_id in DEPRECATED_MODELS

    def suggest_replacement_family(self, model_id: str) -> str:
        """Suggested Claude family for a deprecated model."""
        if "opus" in model_id:
            return "claude-opus"
    ...
    def diagnostics_metadata(self) -> dict[str, str]:
        """Anthropic SDK info for the diagnostics dump."""
        return {"client": f"anthropic=={anthropic.__version__}"}
